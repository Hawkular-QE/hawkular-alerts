...
class Environment(object):
    ...
    def url_metrics(self, metricName):
        metricsClient = HawkularMetricsClient( **{'port':  os.environ['HAWKULAR_PORT'], 'host':  os.environ['HAWKULAR_HOST'], 'username': os.environ['HAWKULAR_USERNAME'], 'password':  os.environ['HAWKULAR_PASSWORD'], 'tenant_id': os.environ['HAWKULAR_TENANT']})
        return metricsClient._service_url((['gauges', metricName, 'raw']))

    def initialize_sample_group_condition(self, metric , metric2):
    ...
    def initialize_sample_group_trigger(self,miq_alert_name, trigger_id):
        trigger = Trigger ()
